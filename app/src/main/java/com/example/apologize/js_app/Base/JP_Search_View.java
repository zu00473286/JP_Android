...
public class JP_Search_View extends BaseActivity{
            ...
            public void run() {
                ...
                myHandler.post(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if(data.size() == 0){
                            new AlertDialog.Builder(JP_Search_View.this)
            ...
            public View getView (final int position, View convertView, ViewGroup parent) {
                ...
                v.setOnClickListener(new View.OnClickListener() {
                    public void onClick(View v) {
                        ...
                        switch (type){
                            case "employeid":
                                if (data.get(position + pageCount).get("employename").toString().split(" ").length > 1){
                                    b.putString("employename",data.get(position + pageCount).get("employename").toString().split(" ")[1]);
                                }
                                if(data.get(position + pageCount).get("employeid").toString().split(" ").length > 1){
                                    b.putString("employeid",data.get(position + pageCount).get("employeid").toString().split(" ")[1]);
                                }
                            ...
                            case "employename":
                                ...
                                break;
                            case "departid":
                                if (data.get(position + pageCount).get("departid").toString().split(" ").length > 1){
                                    b.putString("departid",data.get(position + pageCount).get("departid").toString().split(" ")[1]);
                                }
                                if(data.get(position + pageCount).get("departname").toString().split(" ").length > 1){

                                    b.putString("departname",data.get(position + pageCount).get("departname").toString().split(" ")[1]);
                                }
                                break;
                            case "shift":
                                if (data.get(position + pageCount).get("shiftid").toString().split(" ").length > 1){
                                    b.putString("shiftid",data.get(position + pageCount).get("shiftid").toString().split(" ")[1]);
                                }
                                if(data.get(position + pageCount).get("shiftname").toString().split(" ").length > 1){

                                    b.putString("shiftname",data.get(position + pageCount).get("shiftname").toString().split(" ")[1]);
                                }
                            break;
                            default:
                                break;
