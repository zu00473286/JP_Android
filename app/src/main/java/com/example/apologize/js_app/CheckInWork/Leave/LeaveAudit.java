...
public class LeaveAudit extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.leave_audit);
        findView();
    }
    ...
    void getData() {
        ...
        sb.append("select qjno,qjdate,employeid,employename,absenceno,absencename,udate,udate2,yndate,yndate2,uontime,ynofftime,qjmemo,result from qingjia where qjno=@bid");
                ...
                myHandler.post(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if (connectOK) {
                            ...
                            new AlertDialog.Builder(LeaveAudit.this)
...
//        menu.findItem(R.id.item2).setTitle(Common.toLanguage("手繪簽名"));
    ...
    void uploadData(char[] charArray) {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                if (dbSearch.PostToModify("update qingjia set result='2'", list) == DBSearch.Result.modified) {
                    connectOK = true;
                }
    ...
    void sign() {
        ...
        Intent i = new Intent();
        i.setClass(LeaveAudit.this, DrawBoard.class);
    ...
    void clickToCheck() {
        ...
        sb.append("update qingjia set result='2' where qjno=@bid");
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                connectOK = false;
                //先抓使用者參數設定的特休設定，
                String str = "select leaveSet,NianduBegin from scrit where scname=@account";
                if (dbSearch.JPSearchForGet(str) == DBSearch.Result.fund && saveLeaveSet(dbSearch)) {
                    //更新假單&計算完特休
                    if (dbSearch.JPModifyData(sb.toString()) == DBSearch.Result.modified && calcuLeave()) {
                        connectOK = true;
                    }
                }
                        ...
                        if (connectOK) {
                            ...
                            if (t3.getText().toString().trim().equals("特休")) {
                                new Thread(new Runnable() {
                                    @Override
                                    public void run() {
                                        String str = "update qingjia set result='0' where qjno=@bid";
    ...
    void denied() {
        ...
        sb.append("update qingjia set result='1' where qjno=@bid");
        dbSearch.PutParameter("bid", bid);
    ...
    boolean calcuLeave() {
        ...
        try {
            ...
            if ((t4.getText().toString().trim().substring(0, 3) + Common.workday.substring(3, 7)).compareTo(t4.getText().toString().trim().substring(0, 7)) <= 0) {
                ...
                cmd = "Select bendays from leave where employeid=@employeid and leaveYear=@leaveYear";
                if (dbSearch.JPSearchForGet(cmd) == DBSearch.Result.fund) {
                    JSONObject object;
                    try {
                        for (int i = 0; i < dbSearch.dateArray.length(); i++) {
